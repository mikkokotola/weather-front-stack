...
export class FrontendResources {
    ...
    private createS3Bucket(): IBucket {
        return new Bucket(this.scope, 'weather-app-frontend-bucket', {
            bucketName: 'weather-app-frontend-bucket-2024-03-16',
    ...
    private deployS3Bucket(s3Bucket: IBucket) {
        new BucketDeployment(this.scope, 'deploy-weather-app-frontend', {
            sources: [Source.asset('../weatherfront/dist/weatherfront')],
            destinationBucket: s3Bucket
        });
    }

    private createOriginAccess(s3Bucket: IBucket): S3Origin {
        const originAccess = new OriginAccessIdentity(this.scope, 'OriginAccessControl', {comment: 'Weatherfront App OAI'});
    ...
    private createCfnDistribution(s3Origin: S3Origin): Distribution {
        return new Distribution(this.scope, 'weatherfront-app-cfn-distribution', {
            defaultBehavior: {
                origin: s3Origin,
